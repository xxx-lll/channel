...
function App() {
  ...
  useEffect(() => {
    ...
    if (path.startsWith('/timeline/')) {
      ...
      setCurrentTag(decodeURIComponent(path.split('/timeline/')[1]));
    } else if (path === '/tags') {
      setCurrentPage('tags');
  ...
  const renderContent = () => {
    switch(currentPage) {
      case 'timeline':
        return <Timeline tagName={currentTag} />;
      case 'tags':
        return <Tags />;
      case 'reading':
        return <Reading />;
  ...
  const handleNavClick = (page, tag = null) => {
    setCurrentPage(page);
    if (tag) {
      setCurrentTag(tag);
      window.history.pushState({}, '', `/timeline/${encodeURIComponent(tag)}`);
    } else {
      ...
      <header className="header">
        <nav className="nav">
          ...
          <div className="nav-items">
            <a href="#" className="nav-item" onClick={() => handleNavClick('home')}>首页</a>
            <a href="#" className="nav-item" onClick={() => handleNavClick('tags')}>标签</a>
            <a href="#" className="nav-item">列表</a>
            <a href="#" className="nav-item" onClick={() => handleNavClick('reading')}>阅读</a>
